/**
 * @file gsm_sim800.c
 * @brief Proven SIM800 AT-command and SMS transport
 * implementation.
 * @author Matthithyahu
 *
 * The established command
 * sequence, response timeouts, and UART pacing are
 * intentionally retained
 * because they are verified as stable on the target
 * hardware. Application
 * scheduling is handled by notification_service.
 */
...
static esp_err_t send_at_command(const char *cmd, const char *expected,
                                 uint32_t timeout, bool log_response);
...
static esp_err_t internal_send_sms(const char *phone_number,
...
static esp_err_t parse_sms_message(const char *response, received_sms_t *sms) {
  ...
  if (message_start) {
    ...
    if (message_len > 0 && message_len < sizeof(sms->message)) {
      ...
      int i = 0;
      while (i < message_len && *temp &&
             dest - sms->message < sizeof(sms->message) - 1) {
        if (*temp != '\r' && *temp != '\n') {
          *dest++ = *temp;
        }
        temp++;
  ...
  ESP_LOGI(TAG, "Parsed SMS - Sender: %s, Message: %s", sms->sender,
           sms->message);
  return ESP_OK;
}
...
static esp_err_t clear_uart_buffer(void) {
  ...
  do {
    len = uart_read_bytes(g_gsm_ctx.config.uart_port, temp, sizeof(temp),
...
static esp_err_t wait_for_response(const char *expected, uint32_t timeout) {
  ...
  while ((xTaskGetTickCount() * portTICK_PERIOD_MS - start_time) < timeout) {
    int len = uart_read_bytes(
        g_gsm_ctx.config.uart_port, (uint8_t *)g_response_buffer + pos,
        RESPONSE_BUFFER_SIZE - pos - 1, 100 / portTICK_PERIOD_MS);
...
static esp_err_t send_at_command(const char *cmd, const char *expected,
  ...
  // Send command
  int bytes_written =
      uart_write_bytes(g_gsm_ctx.config.uart_port, at_cmd, strlen(at_cmd));
...
esp_err_t gsm_init(const gsm_config_t *config) {
  ...
  ESP_ERROR_CHECK(uart_driver_install(g_gsm_ctx.config.uart_port,
                                      ...
                                      g_gsm_ctx.config.buf_size, 0, NULL, 0));
  ...
  ESP_ERROR_CHECK(uart_set_pin(g_gsm_ctx.config.uart_port,
                               g_gsm_ctx.config.tx_pin, g_gsm_ctx.config.rx_pin,
  ...
  xTaskCreate(async_sms_task, "gsm_async", 4096, NULL, 5,
              &g_gsm_ctx.async_task);
...
static esp_err_t internal_send_sms(const char *phone_number,
  ...
  int bytes_written = uart_write_bytes(g_gsm_ctx.config.uart_port, full_message,
                                       strlen(full_message));
...
static void async_sms_task(void *arg) {
  ...
  while (1) {
    if (xQueueReceive(g_gsm_ctx.sms_queue, &sms, portMAX_DELAY) == pdTRUE) {
      esp_err_t result = internal_send_sms(sms.phone_number, sms.message,
                                           g_gsm_ctx.config.timeout_ms);
 ...
 */
gsm_status_t gsm_get_status(void) { return g_gsm_ctx.status; }
 ...
 */
void gsm_set_sms_callback(void (*callback)(esp_err_t status,
