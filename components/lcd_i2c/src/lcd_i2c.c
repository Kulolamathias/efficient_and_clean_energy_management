...
static esp_err_t lcd_i2c_send(lcd_handle_t* lcd, uint8_t data) {
  ...
  for (int retry = 0; retry < 3; retry++) {
    err = i2c_master_write_to_device(lcd->config.i2c_port, lcd->config.i2c_addr,
                                     buffer, 1,
 ...
 */
static lcd_error_t lcd_write_nibble(lcd_handle_t* lcd, uint8_t nibble,
                                    bool rs) {
...
lcd_handle_t* lcd_i2c_init(const lcd_config_t* config) {
  ...
  ESP_LOGI(TAG, "LCD initialized: %dx%d, I2C addr: 0x%02X", config->cols,
           config->rows, config->i2c_addr);
...
lcd_error_t lcd_display_control(lcd_handle_t* lcd, bool display, bool cursor,
                                bool blink) {
  if (lcd == NULL) return LCD_ERR_INVALID_ARG;
...
lcd_error_t lcd_entry_mode(lcd_handle_t* lcd, lcd_entry_mode_t direction,
                           bool shift) {
  if (lcd == NULL) return LCD_ERR_INVALID_ARG;
...
lcd_error_t lcd_create_char(lcd_handle_t* lcd, uint8_t location,
                            const lcd_custom_char_t* char_data) {
  ...
  // Set CGRAM address
  lcd_error_t result =
      lcd_write_byte(lcd, LCD_CMD_CGRAM_ADDR | (location << 3), false);
...
lcd_error_t lcd_scroll(lcd_handle_t* lcd, lcd_scroll_dir_t direction,
                       uint8_t count) {
  if (lcd == NULL) return LCD_ERR_INVALID_ARG;
...
lcd_error_t lcd_get_dimensions(lcd_handle_t* lcd, uint8_t* rows,
                               uint8_t* cols) {
 ...
 */
lcd_error_t lcd_print_mixed(lcd_handle_t* lcd, const lcd_print_item_t* items,
                            uint8_t count) {
